...
namespace {
  ...
  void shared_image_delete( void* p ) {
  ...
  int shared_image_index( lua_State* L ) {
  ...
  int shared_image_copy( lua_State* L ) {
    Fl_Shared_Image* si = check_shared_image( L, 1 );
  ...
  int shared_image_reload( lua_State* L ) {
    Fl_Shared_Image* si = check_shared_image( L, 1 );
    F4L_TRY {
      si->reload();
    } F4L_CATCH( L );
  ...
  int shared_image_get( lua_State* L ) {
  ...
  int shared_image_find( lua_State* L ) {
    char const* name = luaL_checkstring( L, 1 );
    int w = moon_optint( L, 2, 0, INT_MAX, 0 );
    int h = moon_optint( L, 3, 0, INT_MAX, 0 );
    F4L_TRY {
  ...
  inline int protected_register_images( void ) {
#if 0
    F4L_TRY {
...
MOON_LOCAL void f4l_shared_image_setup( lua_State* L ) {
  ...
  moon_defcast( L, F4L_SHARED_IMAGE_NAME, F4L_IMAGE_NAME,
                f4l_cast< Fl_Shared_Image, Fl_Image > );
  f4l_new_class_table( L, "Shared_Image", 0, smethods );
  (void)protected_register_images();
}
